...
def main():
    ...
    combinations = (len(rgb_images) * len(SUBSAMPLINGS) * len(QUALITIES) * len(RESTART_INTERVALS))
    current = 0
    ...
    if not out_csv.exists() or out_csv.stat().st_size == 0:
        out_csv.write_text(
            "img_name,subsampling,quality,restart_interval,"
            ...
            for subsampling in SUBSAMPLINGS:
                for quality in QUALITIES:
                    for restart_interval in RESTART_INTERVALS:
                        ...
                        f.write(
                            f"{img_name},{subsampling},{quality},{restart_interval},"
