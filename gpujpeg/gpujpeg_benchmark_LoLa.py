...
ENCODE_PROG_PATH = "build/gpujpeg_encode.exe"
DECODE_PROG_PATH = "build/gpujpeg_decode.exe"
...
def main():
    ...
    if width >= 0: RESTART_INTERVALS = [8]
    if width >= 1920: RESTART_INTERVALS = [16]
    if width >= 3840: RESTART_INTERVALS = [32]
    ...
    combinations = (len(rgb_images) * len(SUBSAMPLINGS) * len(QUALITIES) * len(FORMATS) * len(RESTART_INTERVALS))
    current = 0
                ...
                for quality in QUALITIES:
                    for format in FORMATS:
                        for restart_interval in RESTART_INTERVALS:
                            ...
                            if enc_rc != 0:
                                # unsupported feature combos end here
                            ...
                            if len(decoded_rgb_bytes) != expected_bytes:
                                print(
                                    f"[DEC ERROR] {img_name}: decoded size {len(decoded_rgb_bytes)} != expected {expected_bytes}",
                                    file=sys.stderr)
                                continue
                            ...
                            f.write(
                                f"{img_name},{subsampling},{quality},{format},{restart_interval},"
