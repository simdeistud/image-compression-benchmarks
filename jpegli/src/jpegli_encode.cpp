...
Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (tok == "-w") args.width = std::stoi(need_value("-w"));
        else if (tok == "-h") args.height = std::stoi(need_value("-h"));
        else if (tok == "-i") args.iterations = std::stoi(need_value("-i"));
        else if (tok == "-b") args.benchmark = true;
        else if (tok == "-o") args.output = need_value("-o");
        else if (tok == "-q") args.quality = std::stoi(need_value("-q"));
        else if (tok == "-s") args.subsampling = need_value("-s");
        else if (tok == "-dct") args.dct = need_value("-dct");
        else if (tok == "-entropy") args.entropy = need_value("-entropy");
        else if (tok == "-r") args.restart_interval = static_cast<unsigned int>(std::stoul(need_value("-r")));
        else fail(std::string("unknown argument: ") + std::string(tok));
    }
    if (args.width <= 0 || args.height <= 0) fail("-w and -h must be positive");
    if (args.iterations <= 0) fail("-i must be positive");
    if (args.quality < 0 || args.quality > 100) fail("-q must be in [0,100]");
    if (!(args.subsampling == "420" || args.subsampling == "422" || args.subsampling == "444")) fail("-s must be one of: 420, 422, 444");
    if (!(args.dct == "fast" || args.dct == "float" || args.dct == "int")) fail("-dct must be one of: fast, float, int");
    if (!(args.entropy == "huffman" || args.entropy == "arithmetic")) fail("-entropy must be one of: huffman, arithmetic");
    if (args.output.has_value() && args.benchmark) fail("-o cannot be used together with -b");
    if (args.output.has_value() && args.iterations != 1) fail("-o cannot be used together with -i values different from 1");
    return args;
}
...
std::vector<unsigned char> read_all_stdin() {
    ...
    while (std::cin) {
        ...
        if (got > 0) data.insert(data.end(), buffer.begin(), buffer.begin() + got);
    }
    return data;
}
...
void write_all(const std::vector<unsigned char>& data, const std::optional<std::string>& output) {
    if (!output.has_value() || *output == "-") {
        ...
        if (!std::cout) fail("failed to write to stdout");
        return;
    }
    std::ofstream ofs(*output, std::ios::binary);
    if (!ofs) fail("failed to open output file: " + *output);
    ofs.write(reinterpret_cast<const char*>(data.data()), static_cast<std::streamsize>(data.size()));
    if (!ofs) fail("failed to write output file: " + *output);
}

J_DCT_METHOD to_dct_method(const std::string& dct) {
    if (dct == "fast") return JDCT_IFAST;
    if (dct == "float") return JDCT_FLOAT;
    return JDCT_ISLOW;
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (rgb.size() != expected_size) fail("stdin size does not match w*h*3 for RGB24 input");

        JpegErrorManager jerr;
        jpeg_compress_struct cinfo{};
        cinfo.err = jpeg_std_error(&jerr.pub);
        jerr.pub.error_exit = jpeg_error_exit;
        if (setjmp(jerr.jump_buffer)) throw std::runtime_error(std::string("libjpeg error: ") + jerr.message);
        ...
        for (int i = 0; i < warmup_iterations; ++i) (void)encode_once(&cinfo, args, rgb, nullptr, nullptr, nullptr);

        if (args.benchmark) {
            for (int i = 0; i < args.iterations; ++i) last_output = encode_once(&cinfo, args, rgb, &metrics.setup_time, &metrics.process_time, &metrics.reset_time);
        ...
        for (int i = 0; i < args.iterations; ++i) last_output = encode_once(&cinfo, args, rgb, nullptr, nullptr, nullptr);
        jpeg_destroy_compress(&cinfo);
