...
ENCODE_PROG_PATH = "build/libjpeg-turbo_encode.exe"
DECODE_PROG_PATH = "build/libjpeg-turbo_decode.exe"

SUBSAMPLINGS = ["444", "422", "420"]
QUALITIES = range(0, 101, 10)
DCT_ALGORITHMS = ["int", "fast", "float"]
ENTROPY_ALGORITHMS = ["huffman"]
...
def main():
    ...
    combinations = (len(rgb_images) * len(SUBSAMPLINGS) * len(QUALITIES) *
                    len(DCT_ALGORITHMS) * len(ENTROPY_ALGORITHMS) * len(RESTART_INTERVALS))
    current = 0
    ...
    if not out_csv.exists() or out_csv.stat().st_size == 0:
        out_csv.write_text(
            "img_name,subsampling,quality,dct_algorithm,entropy_algorithm,restart_interval,"
        ...
        for img_path in rgb_images:
            ...
            for subsampling in SUBSAMPLINGS:
                for quality in QUALITIES:
                    for dct_algorithm in DCT_ALGORITHMS:
                        for entropy_algorithm in ENTROPY_ALGORITHMS:
                            for restart_interval in RESTART_INTERVALS:
                                ...
                                enc_cmd = [
                                    ...
                                    "--quality", str(quality),
                                    "--dct_algorithm", dct_algorithm,
                                    "--entropy_algorithm", entropy_algorithm,
                                ...
                                f.write(
                                    f"{img_name},{subsampling},{quality},{dct_algorithm},{entropy_algorithm},{restart_interval},"
