...
std::vector<unsigned char> decode_once(j_decompress_ptr dinfo, const Args& args, const std::vector<unsigned char>& jpeg_bytes, double* setup_time, double* process_time, double* reset_time) {
    ...
    if (static_cast<int>(dinfo->output_width) != args.width || static_cast<int>(dinfo->output_height) != args.height) fail("decoded image dimensions do not match provided -w/-h");
    if (dinfo->output_components != 3) fail("decoded image is not RGB24 after conversion");
...
int main(int argc, char** argv) {
    try {
        ...
        if (jpeg_bytes.empty()) fail("stdin is empty");
        ...
        for (int i = 0; i < warmup_iterations; ++i) (void)decode_once(&dinfo, args, jpeg_bytes, nullptr, nullptr, nullptr);

        if (args.benchmark) {
            for (int i = 0; i < args.iterations; ++i) last_output = decode_once(&dinfo, args, jpeg_bytes, &metrics.setup_time, &metrics.process_time, &metrics.reset_time);
        ...
        for (int i = 0; i < args.iterations; ++i) last_output = decode_once(&dinfo, args, jpeg_bytes, nullptr, nullptr, nullptr);
        jpeg_destroy_decompress(&dinfo);
