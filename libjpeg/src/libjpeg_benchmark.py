...
def build_opt_parameter_sets() -> Iterable[Tuple[Dict[str, str], Dict[str, str]]]:
    for q, subs, dct, entropy, restart in itertools.product(
        QUALITY_VALUES,
        SUBSAMPLING_VALUES,
        DCT_VALUES,
        ENTROPY_VALUES,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark libjpeg encode/decode executables")
    ...
    encode_binary = script_dir / exe_name("libjpeg_encode")
    decode_binary = script_dir / exe_name("libjpeg_decode")
    ...
    for image_path in image_paths:
        ...
        if len(raw_bytes) != expected_raw_size:
            raise ValueError(
                f"raw RGB24 file size mismatch for {image_path}: got {len(raw_bytes)}, expected {expected_raw_size}"
            )
