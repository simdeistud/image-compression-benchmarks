...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            "  --input <path>|-           Selected webp [PATH|stdin]\n"
...
int main(int argc, char *argv[])
{
    ...
    if (benchmark)
    {
        ...
        {
            clock_t t0 = clock();
            unsigned char *rgbi24_output = WebPDecodeRGB(webp_input, webp_input_size, &width, &height);
    ...
    { 
        fprintf(stderr, "Error: Failed to read webp header from: %s\n", webp_input_path);
        return 1;
    }
