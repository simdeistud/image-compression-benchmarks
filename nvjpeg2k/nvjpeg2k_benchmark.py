...
QUALITIES = [1] + list(range(10, 101, 10)) # nvjpeg2k doesn't support quality=0
...
def main():
    ...
    combinations = (len(rgb_images) * len(SUBSAMPLINGS) * len(LOSSY_MODES) * len(QUALITIES)) + (len(rgb_images) * len(SUBSAMPLINGS) * len(LOSSLESS_MODES))
    current = 0
    ...
    if not out_csv.exists() or out_csv.stat().st_size == 0:
        out_csv.write_text(
            "img_name,subsampling,mode,quality,"
            ...
            for subsampling in SUBSAMPLINGS:
                for mode in LOSSY_MODES:
                    for quality in QUALITIES:
                        ...
                        f.write(
                            f"{img_name},{subsampling},{mode},{quality},"
                ...
                for mode in LOSSLESS_MODES:
                    for quality in range(100, 101, 1): 
                        current += 1
                        print(f"Testing combination {current}/{combinations}")
